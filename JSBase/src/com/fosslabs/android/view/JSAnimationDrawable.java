...
public class JSAnimationDrawable extends AnimationDrawable {

	private volatile int duration;// its volatile because another thread will
									// update its value
	private int currentFrame;

	public JSAnimationDrawable() {
		currentFrame = 0;
	}
	...
	public void setDuration(int duration) {
		...
		// we have to do the following or the next frame will be displayed after
		// the old duration
		unscheduleSelf(this);
		selectDrawable(currentFrame);
		scheduleSelf(this, SystemClock.uptimeMillis() + duration);
