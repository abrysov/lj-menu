...
import com.fosslabs.android.view.MosaicLayout.MosaicLayoutParams;
...
public class MosaicAdapter extends BaseAdapter{
	private static final String TAG = "MosaicAdapter";
	
	private Context mContext;
	private ArrayList<Object> mObjects = new ArrayList<Object>();
	private ArrayList<MosaicLayoutParams> mParams = new ArrayList<MosaicLayout.MosaicLayoutParams>();
	...
	public MosaicAdapter(Context context, int resource, int columnCount) {
		mContext = context;
		mResId = resource;
		if(columnCount < 1){
			Funs.getToast(mContext, "MosaicAdapter: ERROR mColumnCount < 1. Can not work");
			mEdit = false;
			return;
		}
	...
	public void addObject(Object object, int width, int height){
		...
		MosaicLayoutParams mp = mPos.addObject(width, height);
		if(mp == null){
	...
	public MosaicLayoutParams getItemParams(int position){
	...
	private class Position{
		...
		public MosaicLayoutParams addObject(int width, int height){
			JSLog.d(TAG, "addObject " + width  + " " + height);
			
			MosaicLayoutParams mp = null;
			if (mX + width -1 < mN){
				mp = new MosaicLayoutParams(mX, mY, width, height);
